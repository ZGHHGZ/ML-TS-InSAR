...
def runGeo(inps):
    ...
    if not inps.isAlexGrid:
        ...
        for infile in inps.prodlist:
            ...
            os.system(cmd)
            if inps.istiff:
                print(str(outFile)+"准备生成tiff")
                if outFile[-3:]=='unw':
                    print('unw')
                    cmd1='gdal_translate -of GTiff ' + outFile +' ' + outFile+ '.tif' +' -b 2'
                elif outFile[-3:]=='cor':
                    cmd1='gdal_translate -of GTiff ' + outFile +' ' + outFile+ '.tif' +' -b 1'
                elif outFile[-3:]=='int':
                    cmd1='gdal_translate -of GTiff ' + outFile +' ' + outFile+ '.tif' +' -b 1'
                elif outFile[-3:]=='omp':
                    cmd1='gdal_translate -of GTiff ' + outFile +' ' + outFile+ '.tif' +' -b 1'
                elif outFile[-7:]=='hgt.rdr':
                    cmd1='gdal_translate -of GTiff ' + outFile +' ' + outFile+ '.tif' +' -b 1'
                elif outFile[-7:]=='los.rdr':
                    cmd1='gdal_translate -of GTiff ' + outFile +' ' + outFile+ '_theta.tif' +' -b 1'
                    os.system(cmd1)
                    cmd1='gdal_translate -of GTiff ' + outFile +' ' + outFile+ '_phi.tif' +' -b 2'

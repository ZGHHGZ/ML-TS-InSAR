...
def main(iargs=None):
    ...
    if not inputs:
        raise RuntimeError(
            'Reference contains no usable IW burst products: {0}. '
            'Run run_00_unpack_topo_reference first and inspect its Sentinel-1 '
            'annotation parsing output.'.format(inps.reference))
